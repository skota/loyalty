defmodule LoyaltyWeb.RewardLive.Index do
  ...
  def handle_event("show_new_modal", _params, socket) do
    changeset = Rewards.change_reward(%Reward{loyalty_program_id: socket.assigns.loyalty_program_id})
    ...
    case Rewards.create_reward(params) do
      {:ok, _reward} ->
        {:noreply, assign(socket,
          rewards: list_rewards(socket.assigns.loyalty_program_id),
          show_modal: false,
          form: to_form(Rewards.change_reward(%Reward{loyalty_program_id: socket.assigns.loyalty_program_id}))
        )}
