defmodule Loyalty.SampleData do
  ...
  def generate() do
    ...
    {:ok, _customer_loyalty_program} =
      Rewards.create_customer_loyalty_program(%{
    ...
    points_earned =
      Decimal.mult(loyalty_program.points_per_dollar, purchase_params["amount_cents"])
      |> Decimal.to_integer()
      |> div(100)
    ...
    {:ok, _purchase} =
      record_purchase_and_award_loyalty_points(purchase_params, customer, loyalty_program.id)
  end
  ...
  def record_purchase_and_award_loyalty_points(purchase_params, customer, loyalty_program_id) do
    ...
    result =
      ...
      |> Ecto.Multi.update(
        :customer,
        Rewards.update_customer_changeset(customer, %{points_balance: points_balance})
      )
      |> Ecto.Multi.insert(:purchase, Rewards.insert_purchase_changeset(purchase_params))
      |> Ecto.Multi.insert(
        :loyalty_points_transaction,
        Rewards.insert_loyalty_points_changeset(loyalty_point_transaction)
      )
      |> Repo.transaction()
  ...
  def create_user_and_team() do
    ...
    {:ok, user} =
      Accounts.register_with_email_password(%{
